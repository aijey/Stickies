...
void doMove(bool &W,bool &A, bool &S, bool &D, sf::CircleShape &shape, int playerNum, sf::RenderWindow &window, int &returnCode)
{
...
int game(sf::RenderWindow &window)
{
    ...
    while (window.isOpen())
    {
        ...
        doMove(W,A,S,D,player1,1,window,returnCode);
        if (returnCode!=0) return returnCode;
        doMove(Up,Left,Down,Right,player2,2,window,returnCode);
        if (returnCode!=0) return returnCode;
        player1CoolDown=0;
        player2CoolDown=0;
        sf::Clock Clock;
